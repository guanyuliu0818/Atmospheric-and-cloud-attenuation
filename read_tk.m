function [tk]=read_tk(ilat,ilong)
...
%
% e.g. tk(:,1:4),hm(:,1) is the air temperature profile 
...
if ((ilat == -1) & (ilong == -1))
    ...
    if isempty(fid)
        fid(1)=fopen('temp_00.bin','r','ieee-be');
        fid(2)=fopen('temp_06.bin','r','ieee-be');
        fid(3)=fopen('temp_12.bin','r','ieee-be');
        fid(4)=fopen('temp_18.bin','r','ieee-be');
    end
    x=zeros(12,4,32);
    lrec=1536;
